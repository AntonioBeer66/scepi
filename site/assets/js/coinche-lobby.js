...
function normalizeLobbies(state) {
  ...
  for (const [id, lobby] of Object.entries(state)) {
    if (id.startsWith("table-") && Number(id.slice(6)) <= PERMANENT_COUNT)
      continue;
    if (typeof lobby.number !== "number") {
      nextNumber =
        Math.max(
          nextNumber,
          ...Object.values(state).map((l) => l.number || 0),
        ) + 1;
      lobby.number = nextNumber;
      changed = true;
    }
    if (lobby.ephemeral === undefined) {
      lobby.ephemeral = true;
      changed = true;
    }
  }
  return changed;
...
function createEphemeralLobby(state) {
  ...
  const id =
    "lobby-" + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
...
function findMySeat(state, clientId) {
  for (const [lobbyId, lobby] of Object.entries(state)) {
    const seatIndex = lobby.seats.findIndex(
      (s) => s && s.clientId === clientId,
    );
...
function seatMarkup(lobbyId, seat, index, clientId, mySeat, lobby) {
  ...
  const busyElsewhere =
    mySeat && (mySeat.lobbyId !== lobbyId || mySeat.seatIndex !== index);
...
function render(grid, state, clientId) {
  ...
  const ordered = Object.entries(state).sort(
...
function init() {
  ...
  grid.addEventListener("submit", (event) => {
    ...
    state[lobbyId].seats[seatIndex] = {
      name: pseudo.slice(0, 18),
      type: "human",
      clientId,
    };
    saveState(state);
  ...
  if (newLobbyBtn) {
    newLobbyBtn.addEventListener("click", () => {
      ...
      if (findMySeat(state, clientId)) {
        window.alert(
          "Quittez votre table actuelle avant d’en ouvrir une nouvelle.",
        );
        return;
      }
